...
async function fetchList() {
  loading.value = true
  try {
    const [listRes, treeRes] = await Promise.all([menusControllerFindAll(query), menusControllerFindTree()])
...
function handleReset() { query.page = 1; query.pageSize = 10; query.keyword = ''; query.type = undefined; query.enabled = undefined; fetchList() }
function openCreate() { formMode.value = 'create'; currentMenu.value = null; formVisible.value = true }
function openEdit(item: MenuModule.MenuItem) { formMode.value = 'edit'; currentMenu.value = item; formVisible.value = true }
...
async function handleRemove(item: MenuModule.MenuItem) {
  const ok = await $asyncBaseConfirm({ title: '删除菜单', content: `确认删除菜单「${item.name}」吗？` })
  if (!ok) return
...
<template>
  <PageContainer fluid>
    <div class="page-shell">
      <ElCard shadow="never" class="toolbar-card">
        ...
        <div class="search-bar">
          <ElInput v-model="query.keyword" class="search-keyword" placeholder="搜索菜单名称/编码/权限标识" @keyup.enter="handleSearch" />
      ...
      <ElCard shadow="never">
        <ElTable :data="list" v-loading="loading" border>
          ...
          <ElTableColumn prop="permission" label="权限标识" min-width="180"><template #default="{ row }">{{ row.permission || '-' }}</template></ElTableColumn>
          <ElTableColumn prop="path" label="路由路径" min-width="180"><template #default="{ row }">{{ row.path || '-' }}</template></ElTableColumn>
          <ElTableColumn label="状态" width="120"><template #default="{ row }"><ElTag :type="row.enabled ? 'success' : 'info'">{{ row.enabled ? '启用' : '停用' }}</ElTag></template></ElTableColumn>
          ...
          <ElTableColumn label="操作" min-width="140" fixed="right">
            <template #default="{ row }">
              ...
              <ElButton link type="danger" :loading="actionId === row.id" @click="handleRemove(row)">删除</ElButton>
            </template>
          </ElTableColumn>
        </ElTable>
    ...
    <MenuFormDialog v-model="formVisible" :mode="formMode" :menu="currentMenu" :parent-options="parentOptions" :loading="submitLoading" @submit="handleSubmit" />
  </PageContainer>
</template>
